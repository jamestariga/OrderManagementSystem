...
Class Action {
	...
	function save_parcel(){
		...
		foreach($price as $k => $v){
			...
			foreach($_POST as $key => $val){
				if(!in_array($key, array('id','item_name','order_number','purchase_date','quantity','price')) && !is_numeric($key)){
					if(empty($data)){
						$data .= " $key='$val' ";
					}else{
						$data .= ", $key='$val' ";
			...
			if(!isset($type)){
				...
				$data .= ", order_number='{$order_number[$k]}' ";
				$data .= ", purchase_date='{$purchase_date[$k]}' ";
	...
	function get_parcel_heistory(){
		...
		if($parcel->num_rows <=0){
			...
			$data[] = array('status'=>'New','date_created'=>date("M d, Y h:i A",strtotime($parcel['date_created'])));
